...
assert.match(html, /淡季单店合格总回款基准 300 元 \/ 店/, '页面应展示更新后的淡季单店总回款基准说明');
...
assert.equal(result.offSeasonRequiredStoresExact.toFixed(1), '388.9', '按公司总成本反推的淡季所需开单精确值不正确');
assert.equal(result.offSeasonRequiredStores, 389, '按公司总成本反推的淡季所需开单数不正确');
...
assert.equal(orderDrivenResult.targetRevenue.toFixed(2), '180000.00', '按计划开单数测算时，目标总回款金额应随滑块变化');
assert.equal(orderDrivenResult.targetGrossProfit.toFixed(2), '110000.00', '按计划开单数测算时，计划利润额应按总回款减公司总成本计算');
assert.equal(orderDrivenResult.targetProfitMarginRate.toFixed(4), '0.6111', '按计划开单数测算时，当前利润率不正确');
...
assert.equal(orderDrivenResult.offSeasonPlannedRevenue.toFixed(2), '180000.00', '淡季计划开单对应回款金额不正确');
assert.equal(orderDrivenResult.revenueGap.toFixed(2), '-63333.33', '淡季计划开单与目标回款的差额不正确');
assert.equal(orderDrivenResult.completionRate.toFixed(4), '1.5429', '淡季计划开单完成率不正确');
assert.equal(orderDrivenResult.peakEquivalentStoresExact.toFixed(1), '600.0', '旺季等价开单精确值不正确');
assert.equal(orderDrivenResult.peakEquivalentStores, 600, '旺季等价开单取整值不正确');
assert.equal(orderDrivenResult.offSeasonRequiredStores, 389, '固定目标下的淡季所需开单数不正确');
